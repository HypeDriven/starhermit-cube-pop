/* Cube Pop — StarHermit platform adapter (browser global: window.CPPlatform).
 * Launch-token lifecycle (fragment read + strip, Bearer, 45-min refresh),
 * profile nickname, and the cloud-save mirror for the checksummed progress
 * document (zip+base64 slot; remote wins on boot; debounced saves with a
 * pagehide flush; sync status). Offline play is unchanged: no token →
 * localStorage only and zero /api calls. Tokens are memory only.
 */
(function (root, factory) {
  var api = factory(root);
  if (typeof module === 'object' && module.exports) module.exports = api;
  else root.CPPlatform = api;
   ...
   * the checksummed progress document JSON. Remote wins on boot (validated by
   * CPSession.loadProgressRaw); saves debounce ~2 s and flush on pagehide/
  ...
  function onProgressSave(docJson) {
